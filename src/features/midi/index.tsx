...
new Array(qwerty.length).fill(null).forEach((_, i) => (dvorakMap[qwerty[i]] = dvorak[i]))
...
function setQwertyKeyConfig(blackKeys: string, whiteKeys: string, octave: number, dvorak = false) {
  ...
  blackNotes.forEach(
    (n, i) => (qwertyKeyConfig[dvorak ? dvorakMap[blackKeys[i]] : blackKeys[i]] = [n, octave]),
  )
  whiteNotes.forEach(
    (n, i) =>
      n !== '' && (qwertyKeyConfig[dvorak ? dvorakMap[whiteKeys[i]] : whiteKeys[i]] = [n, octave]),
  )
}
